...
In previous blogs/videos I showed how we can implement a Numeric instance so we can do arithmetic on many different types, just so long as we create an instance of Numeric to handle them. Now we must implement Numeric for the following type `code`.
...
which means it takes our `code` and a pure function, which it will apply to the Numeric. Unfortunately we end up with an extra layer of Either! Let's see why...
