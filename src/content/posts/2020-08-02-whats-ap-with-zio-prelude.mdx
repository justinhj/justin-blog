...
John has pointed out [before](https://www.reddit.com/r/scala/comments/afor0h/scalaz_8_timeline/eeac71m/) that Ap is not a great ergonomic fit for Scala (currying is much more natural in Haskell). Ap is hard to teach because of this; a lot of Scala developers find currying tricky. It is, however, an important part of the language, and a useful tool for every Scala programmers toolbox.
...
As you'll see in the [slide deck](https://www.slideshare.net/jdegoes/refactoring-functional-type-classes), the goals of prelude are to be the following:
...
2.  Laws. The companion object contains the laws to test the algebra directly. Each object has a 

    ```
    undefined
    ```

     member that can be checked with the package objects 
    ...
    ```
     ...
     method.
3.  Instances. Rather than have instances in a separate file or package, they are also embedded in the companion. It's nice to have them all easily listed in the same place. Most algebras have instances for some appropriate standard library types, as well as some "ZIO" types like 
    ...
    ```
     ...
     and new Prelude types like 

    ```
    undefined
    ```

     which we will get into in a later post. Another thing I like is the code documentation is laid out like this 

    ```
    undefined
    ```

     so that if you want to find, say, all the instances for ZIO you can do do a global text search for 
...
In Cats we extend Functor with Applicative and add the `code` operation to get Monad. Applicative brings us both `code` and `code`. Leaving ap aside for the moment, pure is the important ability to lift a pure value into the context of some effect, represented as a higher-kinded type.
...
IdentityFlatten is the composition of prelude types Associative, Identity and Flatten. Flatten is easy it just gives us the ability to flatten an effect from an `code` to an `code`. For example, the somewhat contrived code below executes a ZIO that is nested inside another, resulting in a `code`. We can use the ZIO Flatten instance to flatten and run the effects.
...
Whilst most types, including ZIO effects, Future, Option and List all have flatMap, we `code` define it in terms of `code` and `code` as follows.
...
Monad also traditionally defines `code`, a way to lift pure values into an effect context. We can do that with Covariant's `code` and IdentityFlatten's `code`.`code` summons an effect out of thin air for us, and we can then use map to sneak our pure value into that effect. Whilst this seems a little tricky, it gives a bit more flexibility. As Adam Fraser puts it, this "also allows you to express constraints on the types of values that can be injected through implementing CovariantSubset instead of Covariant". Subsets were not featured in the talk and I'll talk about those more in a future post once my understanding is more solidified.
...
In the seminal paper Applicative programming with Effects, the first motivating example for applicative programming is the sequence function. You have a list of effects, specifically Haskell IO effects, and you would like to turn them into an `code`. You might recognise this as having the same shape and purpose as Future.sequence from the Scala standard library. sequence is built with its more powerful friend traverse.
...
Let's visit the same problem using our ZIO effects above. One thing I really like about prelude is how combinations of its algebras are mapped to ZIO effects. In this table we have two ZIO effects `code` and `fb : ZIO\[R, E, B]` that are combined in different ways just by changing the algebra.
...
ZIO's implementation of Traverse has eschewed conventional names for some functions in favour or more common words, so for example sequence is just flip, which describes the flipping of the `code` to a `code`. We should be able to just flip our list of ZIOs and execute them using traverse.
...
Whilst ap is certainly not a perfect for Scala, what it does have going for it from a pedagogical point of view is that you can teach Functor, Monad and Applicative as being three ways to map a `code`. The only difference between them is the "shape" of the function you use to do the mapping. With functor the pure function `code` defines the Functor as letting us run a pure function on an effect. With flatMap the function `code` lets us compose two effects together, with the result of one passed to the second. And finally, with Applicative, the `code` function is the building block for running two independent effects together.

With that out of the way, and with the student sufficiently guided through the process of currying and the implementation of map2, map3 etc, we can move on from ap and maybe never look at it again. Applicative is still applicative whether you formulate it with `code` and `code`, with `code`, or with `code`. You can enjoy the benefits of Applicative without adopting the red-headed stepchild we call `code`.
