...
As the [Scala 3 - book](https://docs.scala-lang.org/scala3/book/taste-collections.html) points out, Scala has a rich set of collection classes. As well as `code`, of course, it also has a solid implementation of `code`. If you're not sure what that is, or what is used for, read on and find out, plus even better I will walk through a full implementation of LazyList that can do some magical things.

Scala `code` can represent collections of `code`, stored as a linked list, with the details of the underlying data structure abstracted away. In my video [NonEmptyLists more or less](https://youtu.be/7A2xuRkCZBg) I talked about how you can build a variant of `code` that can only be a collection with `code` items.

In this video, I will present the theory and practice of building a `code` type, that adds the additional capability of controlling `code`.

All the code written in this post, and the accompanying video [The Magic of Lazy Lists](https://youtu.be/laB15gG5bjY) can be found in my new educational Scala library [Duct](https://github.com/justinhj/duct/). In order to produce this implementation I studied the code of the Scala standard library (both the current version and history versions which are less sophisticated but also easier to read), as well as other implementations such as that of the ScalaZ Ephemeral list. The resulting code is a combination of these with some of the best parts of both, as well as taking some advantage of Scala 3 features along the way.
...
`code` is easier to understand if you have a good grasp of different evaluation models in Scala, so let's explore that with a custom class called `code`.

With this implementation, LazyThing is just a wrapper of values, with a get function that returns the value. This is what we call eager, or strict evaluation. When I pass the expression `code` is passed into the `code` constructor it is evaluated immediately and stored in the class. Later when the user calls the get method we find that we just get the value; nothing is evaluated again.
...
The `code` structure is not about always evaluation though, it is about `code` or `code` evaluation. We want to be able to remember the result of evaluated list elements, and never evaluate them again. This `code` is the next step.
...
Now you can see that the value is evaluated only once and we can retrieve it multiple times. Memoization is good because it saves us from recomputing values, but it also means we must be mindful of memory use and hanging on to references to the internal structure of our `code` so as not to consume memory that is no longer needed.
...
Secondly, we can replace the manual memoization code that remembers the evaluated value with `code` which does the same thing but, again, is built into the compiler.
...
Let's begin by representing the `code` as a `code`, which will be the object through which users interact with the collection.
...
Of note here is the `code` variance notation. It's important to know about and understand variance when making libraries in Scala, slightly less important when writing application code. A short explanation of variance is that it is short for "variance under inheritance".

Let's say we have a type `code` and two other subtypes of Loan, `code` and `code`. If you have some function that takes Loan and prints the outstanding balance, you would expect through normal rules of inheritance to be able to pass in a Credit card or an amortized loan in place of the Loan. You can use a subtype of loan wherever the compiler is expecting a loan. That is what is known as behavioural subtyping.
...
`code` will have a companion object containing all the static methods that will be used to create and manipulate lazy lists. The first thing we need is a representation of empty list. We add that to a new companion object.
...
Lazy list has the type Nothing. Nothing is at the bottom of Scala's type hierarchy meaning it is the subtype of everything. Now it's not a useful type in itself, because you can't do anything with it, but it is really useful in this context... our empty list is a singleton value shared by all lazy lists, we only need one. Why does this work? Because of the variance annotation above. We said that a list of subtypes of `code` would be acceptable as list of `code`.

So now we are able to create lazy lists with nothing in them using `code`. The next step is to be able to create lists with elements inside. We will call this the `code` method, as it will be used to construct lists one lazy element at a time.
...
In Scala you can construct lists using the so-called `code` operator `code`. For example:
...
This is convenient so Scala's standard `code` also implements this using the syntax `code`. Let's do the same for Duct. There are two things to note here:

1.  To make this work we want #:: to be a right associative function that 

    ```
    undefined
    ```

    's a new head for the list to the tail which is to the right
2.  The type of the operation should be a cons operation on a list.

To append `code` to the list `code` we need to write `code` and we want the compiler to evaluate this as:
...
[scala.collection.immutable.LazyList.scala](https://github.com/scala/scala/blob/39148e4ec34a5c53443dd1b25ceec2308cd097fe/src/library/scala/collection/immutable/LazyList.scala#L1136)

With Scala 3 we can simply implement this as an extension method on the `code` trait. Much nicer.
...
In Scala you implement pattern matching on a particular type by implementing `code` on an object with that types name, in our case `code`.
...
Another useful function is `code`, which you can use to execute some action across the lazy list. This function highlights a couple of interesting things.
...
And you can use it as follows. Note that I'm using the [LazyList.apply](https://github.com/justinhj/duct/blob/2ebe7e3ea25edf4c668ca6e044959396ae3cb265/core/src/main/scala/org/justinhj/duct/datatypes/LazyList.scala#L123) method here is a convenience to create a lazy list from a variable argument list.
...
Another part of the implementation worth looking at is dropping elements that pass or fail some filter, namely `code` and `code`. Let's first think about what the semantics are here in terms of laziness.

1.  Given a lazy list and a filter function we want the user to be able to iterate through them 
...
We have to be careful about laziness then. Let's first think about `code`. This takes lazy list with all the failing elements dropped.
...
Now since we want this to work on many elements potentially, it is important to be tail recursive. With `code` we can take list such as `code` and drop all elements less than 3. What we get back is LazyList beginning with 3.

Take a moment to think about which elements have been evaluated at this point.

Whether you reason about it by looking at the code or thinking about it semantically, the answer is that the 3 is evaluated and the `code` elements are in a lazy tail. `code` then will evaluate elements `code`.

Once you implement `code` it can be used to implement `code` with the requirements we came up with above.
...
`code` provides a lazy list with a head of type `code`. When the use takes the tail they get the same thing and so on forever. This gives us a definition of an infinitely repeating constant.

`code` shows how we can incrementally generate numbers from some starting value n. Note that the tail is a function that takes input from the previous call; in this way we can pass information through an infinite chain of computation!

`code` is a generalisation of this allowing you to take some function that creates a new A from the previous one, forever.

Of course, we don't want to actually evaluate infinite lists because we don't have time for that, so you would use [take](https://github.com/justinhj/duct/blob/2ebe7e3ea25edf4c668ca6e044959396ae3cb265/core/src/main/scala/org/justinhj/duct/datatypes/LazyList.scala#L57) and drop and other filtering mechanisms to work with only the values you are interested in. As we will see, there are times when you don't know how many of a thing you need and it may be expensive to generate them, so `code` evaluation is what we want.
...
1.  ```
    ...
    ```
     ...
     will iterate over the large list, doing expensiveCalculation 10m times and making a new list of 10m elements.
2.  ```
    undefined
    ```

     will walk that new list and create a new list with up to 10m elements that pass the filter.
3.  ```
    undefined
    ```

     will take those elements and create a new list after calling expensiveCalculation2 on each element
4.  ```
    undefined
    ```

     will drop all elements after the 10th one
5.  ```
    undefined
    ```
...
Whilst this kind of code is not typical, you are hopefully not working with lists this big, but if the use case requires it, then lazy lists provide a `code` much more efficient way of working.
...
1.  \[@1] 
    ...
    ```
     ...
     takes the large list and returns a lazy list where, when evaluated, head will have expensiveCalculation applied to it. This is O(1).
2.  \[@2] 
    ...
    . Let's pretend the filter is true because a &lt; 10 and we return a new lazy list with the filter but paused at the first element.
3.  \[@3] 
    ...
    ```
     ...
     will take that list and again, return a new lazy list that is unevaluated and ready to run expensiveCalculation2 if anyone asks.

Observation... we are turning our list of values into a `code`. This takes up more memory than a list of values because each step is wrapped in a Function object.
...
1.  \[@4] 
    ...
    ```
     ...
     will now return a lazy list that keeps track of a counter and stops (returns an empty tail) when it runs out, so we set a bound on our computation.
2.  \[@5] 
    ...
    ```
     ...
     okay now we're going to do a bit more work. sum calls 

    ```
    undefined
    ```

     (see below), which by definition must evaluate all the items and combine them to a single result
...
1.  \[@6] Now more serious evaluation will happen. What we have at this point is a sort of stack of computations for each successive element. We will call expensiveCalculation1 and expensiveCalculation2 

    ```
    undefined
    ```

     to evaluate the 10 elements.

This is all rather hard to conceptualize, so here's a picture that may help. The call stack shown in the middle of the `code` shows that the lazy list we evaluate consists of a stack of function calls that are waiting to happen.
...
Some algorithms require you to provide a list of things but you don't know how many things you need in advance. Here's an example that appears in the paper [Applicative Programming with Effects](https://www.staff.city.ac.uk/~ross/papers/Applicative.html) that transposes a matrix.

You can see this code also in my post about the paper at [What's Ap?](./2020/04/04/whats-ap.html), although the coverage there is more about how this operation can be written in "the applicative style".
...
In order to implement this a nicely functional, declarative way, we first need a helper function `code` that takes two empty lists and lets us combine them with a function.
...
For a stack safe function I present `code`.
...
This is a so-called aggregate function that takes a collection, in this case, iterates over it and produces some aggregate value. The supplied function from the user is applied to each element along with some accumulating value. In the case of this implementation, the foldLeft recursive call is in `code`which means we can assume it uses tail call optimization. We add the annotation to tell the compiler we think so, and it will both complain if it is not eligible.
...
Note that the problem here is that the recursive call is not a tail call position, in this case, the user function `code` is. That means we can't use the tailrec annotation and it will not be tail call optimized.

Can we infer from this situation that `code` is useless? No actually. It has a property that foldLeft does not, that of being able to `code`. Just like with fusion of operations, the early termination of `code` can be used to save us work, and make code more efficient.
...
This example code uses `code` to find "tuna" in a list of fish.
...
Functional Programming in Scala (aka the red book) - has a great chapter on lazy lists[Functional Programming in Scala - Manning Press](https://www.manning.com/books/functional-programming-in-scala)
