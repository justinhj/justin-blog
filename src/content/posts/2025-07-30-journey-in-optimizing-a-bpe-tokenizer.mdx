...
-   **Open-Vocabulary**
     ...
     Part of the token set is the basic characters of each language so you can represent every word.
-   **Rare words**

     Because the vocabulary set is open it means any rare word is handled.
-   **Translation of Novel Words**

     The model can translate and generate words it has not encountered before by composing them from sub-word units.
...
Because the body text required sequential access and the ability to quickly remove elements I used a singly linked list, or `code`. This has the desirable properties of sequential access and O(1) deletions.

`code` has the lowest memory overhead of all std C++ containers (a single pointer to the next element.
...
Sometimes you need to use multiple data structures to support a use case with conflicting requirements. For this purpose I used the `code`.
...
Index 1 explanation: Same as above except the outcome is different because of the implementation of `code`.
