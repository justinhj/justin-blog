...
BTW you can check out the video here instead:[Functional Justin - Another Angle on Monad Transformers](https://youtu.be/tEnZZlOn-jI)
...
The `code` type is a simple map for strings to values that we will use as a symbol table so that variables can be looked up at runtime. The`code` syntax indicates that the return type is a context function. An earlier blog discusses that, but in short, it allows us to thread our Env symbol table through the computation easily.
...
Those arithmetic operators you see are operating not on integers, doubles or some other concrete type, but are working on a type A that has a Numeric instance. You may wonder then how that `code` operator knows what to do? The answer is that I implemented an instance of Numeric for the type `code`.
...
This is not very interesting on its own but if you make a program from Writers, sequencing them together using the Monad's flatMap operation for example, then the end result consists of a final value and a log for `code`.

But since I already committed to using Either, if I change the type to Writer then I would lose the ability to handle errors. Instead what I want is to keep the Either effect and `code` it with the capability of the Writer monad.
...
It's not straightforward because the WriterT wrapped type must be`code` and we had an F\[A]. That is why I need to use the map operation to take any value the Either may have and combine it with an empty log. Here we assume the log is a list of strings and Scala is able to infer that too.
...
`code` is a method on the WriterT data type itself, and it takes advantage of the log types monoid to combine this new log entry with any prior ones.
...
For example, the expression `code` would be logged as `"Val 10"`. Having a step-by-step log of your application has various uses including the following.
...
Again the change is virtually mechanical. We lifted our old code and added the tell call to add some logging information. When we view variable lookups in the log you will see something like `code`written as `code` where 7 is its value in the symbol table.
...
The remainder of the program involves expressions like this one. We use the `code` operator to add two other expressions together. How that works is a combination of operator overloading, extension methods and implementing an implicit implementation of Numeric for our new WriterT return type.
...
Note that by nesting the flatMaps we have access to a,b and the result of the computation so we can put all of that into the tell call, resulting in a log like `code`.

There's nothing really wrong with this implementation, but it's important to always think about the principle of least power. Did I really need a Monad here? Well in fact there is a great function for applying a computation across two different effects and that is`code`. It also requires only Applicative, so I can use that instead.
...
Just like with WriterT we also would benefit from a lift function that lets us take any instance of F and wrap it. Here I'm saying if you have some effect `code` I will give you a ReaderT that wraps it. You can run it with some environment and it will yield that `code` again.
...
Here you can see that our program has to be run sort of inside out. `code` gives us a `code`. By calling `code` I get the`code` inside it, which I can then run by passing the environment.
...
[Reader Writer example in Scalaz](https://github.com/justinhj/ZPurePlay/blob/master/src/main/scala/EvalScalaz.scala)

[Reader Writer example in Typelevel Cats](https://github.com/justinhj/ZPurePlay/blob/master/src/main/scala/EvalCats.scala)
...
[Zio Prelude](https://github.com/zio/zio-prelude) is a new library that acts as a sort of add on library to[ZIO](https://github.com/zio/zio) (a zero-dependency Scala library for asynchronous and concurrent programming) which provides an alternative approach to functional abstractions in Scala.

For the purposes of this blog I'm interested in particular experimental data type within the library called `code`. ZPure has 6 type parameters and supports the operations of monads, applicatives and functors, albeit the names are changed and some of the abstractions too.

From Prelude's own source code "`code` can be used to model a variety of effects including context, state, failure, and logging.", let's see how it compares to monad transformers to implement the program above.
...
-   ```
    ...
    ```
     ...
     Logging. The type of logs that this effect produces, analogous to Writer
-   ```
    undefined
    ```

     State. There are two S's because the type encodes an input and output state type
-   ```
    undefined
    ```

     Reader. The type of the read-only environment
-   ```
    undefined
    ```
...
-   ```
    ...
    ```
     ...
     Value. The type of the happy path computed value

Just as before all I need to do is change my programs effect type and modify the implementation accordingly. The code for this section can be found [here](https://github.com/justinhj/ZPurePlay/blob/master/src/main/scala/EvalZpure.scala).
...
These types will represent the `code` or error, and `code` or log. We can encode our symbol table using the `code` parameter.
...
First I use the ZPure.environment to summon the symbol table and flatMap over it so we can access it as a concrete value `code`.
...
The final step is to add a log. Since I would like the log to show both the variable name and the actual value I have to nest it inside a flatMap so we can access the success value. The last thing to note here is the `code` which helps the type system make a ZPure of the right type.
...
The final component is to make an instance of Numeric. Prelude takes the approach to naming that concepts should be plain English where possible, so it uses `code` instead of `code` or `code` and similarly `code` instead of applicative terminology.
...
There are techniques and libraries to make their use more convenient. Although I have not used it [Cats MTL](https://github.com/typelevel/cats-mtl) offers solutions to some of the problems, but it is not widely used.
